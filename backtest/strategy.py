...
from typing import Dict, Type, Any, ClassVar, List, Optional
...
class RSI_Strategy(BaseStrategy):
    name = "rsi_reversion"
    param_grid: ClassVar[List[Dict[str, Any]]] = [
        {"period": 14, "buy_threshold": 30.0, "sell_threshold": 70.0},
        {"period": 14, "buy_threshold": 35.0, "sell_threshold": 65.0},
        {"period": 21, "buy_threshold": 30.0, "sell_threshold": 70.0},
    ]
...
class Trend_Momentum_Strategy(BaseStrategy):
    ...
    param_grid: ClassVar[List[Dict[str, Any]]] = [
        {"macd_fast": 12, "macd_slow": 26, "macd_signal": 9, "rsi_period": 14, "rsi_buy_max": 65.0},
        {"macd_fast": 8, "macd_slow": 21, "macd_signal": 9, "rsi_period": 14, "rsi_buy_max": 70.0},
    ]
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        # Signals: MACD bullish AND RSI not overbought
        return (macd_bullish & (rsi < self.params.rsi_buy_max)).astype(int)


class EMA_Crossover_Params(StrategyParams):
    fast: int = 10
    slow: int = 30


@register_strategy
class EMA_Crossover_Strategy(BaseStrategy):
    """EMA trend following crossover"""
    name = "ema_crossover"
    param_grid: ClassVar[List[Dict[str, Any]]] = [
        {"fast": 5, "slow": 20},
        {"fast": 10, "slow": 30},
...
class Donchian_Breakout_Params(StrategyParams):
...
class Donchian_Breakout_Strategy(BaseStrategy):
    ...
    def get_params_class(self):
        return Donchian_Breakout_Params

    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        highs = df["high"] if "high" in df.columns else df["close"]
        lows = df["low"] if "low" in df.columns else df["close"]
        upper = highs.rolling(self.params.entry_lookback).max()
        lower = lows.rolling(self.params.exit_lookback).min()

        entry = df["close"] > upper.shift(1)
        exit_ = df["close"] < lower.shift(1)
...
class Bollinger_Breakout_Volume_Params(StrategyParams):
    bb_period: int = 20
    bb_std: float = 2.0
...
@register_strategy
class Bollinger_Breakout_Volume_Strategy(BaseStrategy):
    """Bollinger upper breakout with volume confirmation (trend)"""
    name = "boll_breakout_volume"
    param_grid: ClassVar[List[Dict[str, Any]]] = [
        {"bb_period": 20, "bb_std": 2.0, "volume_period": 20, "volume_factor": 1.2, "exit_on_mid": True},
        {"bb_period": 20, "bb_std": 2.5, "volume_period": 20, "volume_factor": 1.0, "exit_on_mid": True},
    ]

    def get_params_class(self):
        return Bollinger_Breakout_Volume_Params
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        avg_vol = df["volume"].rolling(window=self.params.volume_period).mean() if "volume" in df.columns else None
        vol_ok = (df["volume"] > (avg_vol * self.params.volume_factor)) if avg_vol is not None else pd.Series(True, index=df.index)

        breakout = (df["close"] > upper) & vol_ok
        if self.params.exit_on_mid:
            exit_ = df["close"] < mid
        else:
        ...
        holding = False
        for i in range(len(df)):
            if not holding and bool(breakout.iloc[i]):
                holding = True
            elif holding and bool(exit_.iloc[i]):
                holding = False
            position.iloc[i] = 1.0 if holding else 0.0
